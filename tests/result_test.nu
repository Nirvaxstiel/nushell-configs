...
let result_test = [
    { name: "ok wraps value", run: { assert-equal (ok 42) { kind: "ok" value: 42 }; assert-true (result-is-ok (ok 42)) } }
    { name: "err carries error", run: { assert-equal (err "x") { kind: "err" error: "x" }; assert-true (result-is-err (err "x")) } }
